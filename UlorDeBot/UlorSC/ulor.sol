...
contract Todo {
    ...
    function createProperty(string text) public onlyOwner {
        tvm.accept();
        m_count++;
        m_tasks[m_count] = Task(m_count, text, now, false);
    }

    function updateProperty(uint32 id, bool done) public onlyOwner {
    ...
    function deleteProperty(uint32 id) public onlyOwner {
